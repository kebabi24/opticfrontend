    ...
    return {
        validate(input: ValidateInput<DateOptions, DateLocalization>): ValidateResult {
            ...
            const opts = Object.assign({}, {
                // Force the format to `jj-mm-aaaa` as the default browser behaviour when using type="date" attribute
                format: (input.element && input.element.getAttribute('type') === 'date') ? 'jj-mm-aaaa' : 'MM/DD/YYYY',
                message: '',
            }, input.options);
