...
export class EditTaskComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.activatedRoute.params.subscribe((params) => {
        const id = params.id
        this.taskService.getOne(id).subscribe((response: any)=>{
          console.log(response.data)
          this.taskEdit = response.data.task
          this.mvdataset = response.data.details
         
           // console.log(this.taskEdit.date01, this.date2)
         
          this.initCode()
          this.loadingSubject.next(false)
          this.title = this.title + this.taskEdit.tk_code
        })
    })
    this.initmvGrid()
  }
  ...
  createForm() {
    ...
    this.taskForm = this.taskFB.group({
      tk_code: [this.taskEdit.tk_code, Validators.required],
      tk_desc: [this.taskEdit.tk_desc ,  Validators.required],
      tk_um: [this.taskEdit.tk_um ,  Validators.required],
      tk_price: [this.taskEdit.tk_price ,  Validators.required],
  ...
  addtask(_task: Task, details: any) {
    ...
    this.taskService
      ...
      .update( {Task:_task, details}, this.taskEdit.id).subscribe(
    //  .subscribe(
        ...
        () => {
          ...
          this.loadingSubject.next(false);
          this.reset();
          this.router.navigateByUrl("/task/list-task");
        }
      );
  }
