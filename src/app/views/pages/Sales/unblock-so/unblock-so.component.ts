...
@Component({
  selector: 'kt-unblock-so',
  templateUrl: './unblock-so.component.html',
  styleUrls: ['./unblock-so.component.scss']
})
export class UnblockSoComponent implements OnInit {
 ...
 customer: any
  ...
  createForm() {
      ...
      this.soForm = this.soFB.group({
          ...
          so_stat: [ this.saleOrder.so_stat ],
  ...
  onSubmit() {
      ...
      var stat = null;
      if (controls.so_stat.value == "") { stat = null}
      this.saleOrderService
          .updateSo({ so_stat: stat, so__chr01 : controls.so__chr01.value, so__chr02: this.user.usrd_user_name }, this.soServer.id)
          .subscribe( //(res) => {
            ...
            () => {
                ...
                this.loadingSubject.next(false)
                this.router.navigateByUrl("/sales/unblock-so")
